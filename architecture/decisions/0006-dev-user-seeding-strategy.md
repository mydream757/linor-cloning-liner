...
- 시드 코드:
  ...
  ```
- **`develop/prisma.config.ts`의 `migrations.seed` 항목**으로 `pnpm prisma migrate dev` 시 자동 실행. Prisma 6까지는 `package.json`의 `prisma.seed`에 두었으나, Prisma 7부터 `prisma.config.ts`로 이동되었다 ([notes/prisma7-config-migration.md](../notes/prisma7-config-migration.md) 참조)
