...
- **Chat CRUD**
  ...
  - 목록 표시 (두 축, 병존):
    - **사이드바 Project 트리**: 각 Project 항목을 caret으로 펼치면 해당 Project의 Chat 중첩 표시. "무엇이 어디에 속했는지" containment를 UI로 명시. 모든 뷰 공통.
    - **사이드바 "최근 기록" 섹션**: 현재 user의 cross-project 최신 Chat N개를 최신순으로 노출. Liner 뷰 활성 시만 표시. **2차 후보였던 "서비스 간 최근 탐색"을 MVP로 승격** (2026-04-20 결정).
...
| 결정 | 내용 | 근거 |
|---|---|---|
| LLM 프로바이더 | Anthropic + Gemini 듀얼 (1급) | 비용·가용성 유연성 + 두 프로바이더 비교 학습. 둘 다 공식 SDK 저수준 직접 사용. 프로바이더 통합 추상화(AI SDK / Vercel AI SDK / LangChain) 금지. 상세는 [ADR-0013](../../architecture/decisions/0013-llm-provider-abstraction.md) |
| SSE 구현 방식 | 저수준 직접 구현 (Web Streams API + fetch/ReadableStream) | 프로젝트 핵심 가치 2번(실시간 UX). 고수준 추상화 사용 시 학습 가치 소멸. 클라이언트는 EventSource 대신 fetch+ReadableStream (Q6 해소) |
...
|---|---|---|---|---|---|
| Q1 | 어시스턴트 응답의 영속 전략: 완료 후 일괄 저장 vs 스트리밍 중 incremental persist | 1차는 완료 후 저장으로 시작하되, 긴 응답에서 브라우저 이탈 시 데이터 손실 리스크 존재. 실사용 후 판단 | 구현 D-stage에서 | Developer | **해소 (D2)**: 완료 후 일괄 저장 채택. 스트리밍 시작 시 빈 assistant Message 레코드 생성, 스트림 종료 시 최종 content/citations로 업데이트. 중단·오류 시 브라우저가 이탈하면 해당 응답 유실 — 실사용 후 incremental persist로 전환 재검토 |
| Q2 | 대화 맥락(이전 메시지) 전달 시 컨텍스트 윈도우 관리 전략 (전체 전달 / 최근 N개 / 토큰 수 기반 잘라내기) | 초기에는 전체 전달로 시작 가능하지만, 긴 대화에서 토큰 한도 초과 리스크. 실측 후 결정 | 구현 D-stage에서 | Developer | **해소 (D2, 잠정)**: 전체 메시지 전달. 시스템 role만 필터링. 실측 후 토큰 한도 근처에서 최근 N개 잘라내기로 재검토 |
| Q3 | Chat 목록 UI 위치: Liner 뷰 메인 패널 내 좌측 vs 사이드바 확장 | 사이드바에는 이미 Project 목록이 있어 공간 경합. 디자인 의사결정 영역 | 디자인 단계 | Designer | **재해소 (디자인 v0.2, 2026-04-20)**: Project 트리 하위 Chat 펼침(containment 표시) + 별도 "최근 기록" 섹션(cross-project recent, Liner 뷰 활성 시만) 병존. 초기 v0.1은 단일 "최근 기록" 섹션으로 현재 Project의 Chat만 노출해 라벨-실체 괴리가 컸음 |
| Q4 | 출처 배지 클릭 시 UI: 인라인 팝오버 vs 하단 패널 vs 사이드 패널 | 디자인 의사결정 영역. 기능 4에서 실 Reference 연결 시 UI가 달라질 수 있음 | 디자인 단계 | Designer | **해소 (디자인 v0.1)**: 우측 399px 출처 패널 |
| Q5 | 메시지 내 마크다운 렌더링 범위 (plain text만 vs 기본 마크다운 vs 코드 블록 포함) | 렌더링 라이브러리 선택이 번들 사이즈에 영향. 최소 시작 후 필요 시 확장 | 구현 D-stage에서 | Developer | **해소 (D3)**: `react-markdown` 도입. H1~H3, 단락, 리스트(ordered/unordered), 인라인 코드, 코드 블록(`pre`), 링크, 강조(strong/em)를 지원. 각 요소에 디자인 토큰(`text-heading-N`, `text-body` 등)을 바인딩. GFM 표 등 확장은 미도입 — 필요 시 2차 후보 |
| Q6 | SSE 클라이언트 구현 방식: EventSource API vs fetch + ReadableStream | EventSource는 GET만 지원하는 제약이 있고, POST body로 메시지를 보내려면 fetch 기반이 필요할 수 있음 | 구현 D-stage에서 | Developer | **해소 (D2)**: fetch + ReadableStream 채택. POST body로 메시지 전송 필요 + AbortController 기반 중단 지원 이점 |
| Q7 | Anthropic의 citations 기능 활용 vs 시스템 프롬프트 기반 `[n]` 마커 생성 | API 기능 가용성과 제어 수준에 따라 달라짐. 실제 SDK 통합 시 판단 | 구현 D-stage에서 | Developer | **해소 (D2)**: 시스템 프롬프트 기반 `[n]` 마커 + regex 파싱. 프로바이더 듀얼(Anthropic/Gemini) 지원 결정(ADR-0013)에 따라 프로바이더별 citations 기능에 종속되지 않는 공통 전략 선택 |
| Q8 | 스트리밍 중 어시스턴트 메시지의 `updated_at` 갱신 빈도와 방식 | 도메인 모델이 "청크 단위로 incremental persist"를 언급하지만, Q1과 연동. 1차 완료 후 저장이면 이 질문은 해소 | Q1 결정 시 | Developer | **해소 (D2)**: Q1 결정(완료 후 일괄 저장)에 따라 자동 해소 |

### 추가로 부상한 결정

| 결정 | 내용 | 근거 |
|---|---|---|
| LLM 프로바이더 추상화 | Anthropic + Gemini 듀얼 (1급) | 비용 제약 + 비교 학습. 상세는 [ADR-0013](../../architecture/decisions/0013-llm-provider-abstraction.md) |
...
| D4 | 사이드바 Project 트리 + Chat 관리 + 최근 기록 | Chat 전환/생성/삭제 동선 + containment UI | Project 항목 caret 펼침 (현재 Project 기본 열림), 각 Project 하위 Chat 중첩 렌더 (max-height 240px 내부 스크롤, 16px 들여쓰기), **Project 항목 내부 "+" 버튼**(⋯ 왼쪽, hover 시 노출)으로 해당 Project에 blank Chat 생성, 첫 메시지 시 서버에서 제목 자동 갱신, 전역 "최근 기록" 섹션 (Liner 뷰 활성 시, cross-project recent N개, Chat 생성 버튼 없음), Chat 항목 (선택/hover/컨텍스트 메뉴), Chat rename, 삭제 confirm | 출처, **미할당 Chat(기능 4로 연기)** |
...
- 0.5 (2026-04-21): D5·D6 반영. D5에서 인라인 `[n]` 배지(rehype 플러그인 기반) + 우측 399px 출처 패널 구현 완료. §7 Q5(마크다운 렌더링 범위) 해소 — react-markdown 채택 내역 기록. D6 종합 검증 단계에서 Minor/Nit(citations double JSON round-trip 제거), 남은 한계 2건을 기술 부채(T-004 에러 재시도 시 user 메시지 DB 중복 저장, T-005 ResponseActions stub 아이콘 미구현)로 등록. 기능 3 전체 완료 상태로 `features.md` 갱신.
- 0.4 (2026-04-20): D4 스코프 재설계 — 사이드바를 **Project 트리 + 별도 "최근 기록" 섹션** 구조로 변경. v0.3에 명시된 단일 "최근 기록" 섹션(현재 Project의 Chat만 보여주는 project-scoped 리스트)은 라벨("최근 기록")과 실체(project-scoped)가 불일치해 "무엇이 어디에 포함되는지" UI 투명성이 부족했다. Project 트리로 containment를 명시하고, 라벨 의미를 원래대로 cross-project recent로 되돌린다. §3 Chat CRUD 목록 재작성, §7 Q3 재해소, §8 D4 재정의. features.md 2차 후보였던 "서비스 간 최근 탐색"을 MVP로 승격. **동일 일자 추가 수정**: Chat 생성 진입점을 "최근 기록" 섹션 상단 전역 "+ 새 대화"에서 Project 항목 내부 "+" 버튼으로 이동 — 사용자 피드백("어느 Project에 추가되는지 UI로 불명확"). 전역 "+ 새 대화"는 기능 4에서 미할당 Chat 지원과 함께 부활 예정(Asset 미할당과 top-level 라우트 아키텍처 공유). 첫 user 메시지 시 서버에서 제목 자동 갱신 로직 추가.
- 0.3 (2026-04-20): D2(SSE 스트리밍 인프라) 구현 반영. LLM 프로바이더를 Anthropic 단독에서 Anthropic + Gemini 듀얼(둘 다 1급)로 확장 — 비용 제약과 비교 학습 가치 반영([ADR-0013](../../architecture/decisions/0013-llm-provider-abstraction.md)). §5 테이블에 프로바이더 결정·SSE 클라이언트 방식 업데이트. §7 열린 질문 Q1/Q2/Q6/Q7/Q8 해소 상태 기록. 저수준 직접 구현 원칙은 유지 — 프로바이더 통합 추상화 라이브러리(AI SDK 등)는 계속 금지.
