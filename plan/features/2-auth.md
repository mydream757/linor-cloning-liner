...
- **NextAuth 설정**
  - Google OAuth 프로바이더 (기본)
  - 개발용 Credentials 프로바이더 (`NODE_ENV === 'development'`에서만 활성화, [ADR-0012](../../architecture/decisions/0012-dev-credentials-provider.md))
  - 세션 전략: JWT ([ADR-0011](../../architecture/decisions/0011-auth-session-and-route-protection.md))
  - NextAuth가 요구하는 DB 테이블 (Account, Session 등) 추가
- **로그인 페이지**
  - 별도 페이지(`/login` 또는 NextAuth 기본 경로)
  - "Google로 계속하기" 버튼 (항상 표시)
  - 개발 모드: 이메일/비밀번호 입력 폼 추가 표시 (seed 계정 `dev@local` / `dev-password`)
  - 앱 로고/이름 + 간단한 안내 문구
  - 다크 모드 테마 일관성 유지
- **로그아웃**
...
| ~~이메일/비밀번호 인증 (Credentials)~~ | → **개발용 최소 Credentials로 스코프 변경** ([ADR-0012](../../architecture/decisions/0012-dev-credentials-provider.md)). 레포 클론 시 Google Cloud 설정 없이 즉시 실행 가능하도록. 회원가입/비밀번호 찾기/이메일 인증은 여전히 제외 |
...
## Changelog
- 0.3 (2026-04-16): 개발용 Credentials 프로바이더 스코프 추가 (ADR-0012). 레포 클론 시 Google Cloud 설정 없이 seed 계정(`dev@local`/`dev-password`)으로 즉시 로그인 가능. 비-스코프에서 Credentials 항목을 개발용 최소 버전으로 재분류. 회원가입/비밀번호 찾기/이메일 인증은 여전히 제외.
