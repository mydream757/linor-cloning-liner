...
### 시나리오 5-a: 빈 Document Asset 직접 생성 (보조 플로우)

1. 사이드바 또는 Write 뷰 진입 화면에서 "새 Document" 버튼 클릭
2. 빈 Document Asset이 생성된다 (`type: document`, `origin_chat_id=null`)
3. Document 목록(사이드바 "최근 기록" 재사용)에 나타난다. 편집은 기능 5(Write 뷰)에서 한다
4. Project 내에서 생성하거나, 미할당 상태로 생성하는 두 경로 모두 가능하다

### 시나리오 5-b: Chat 결과를 Document로 포워딩 (주 플로우)

1. 사용자가 Liner 뷰에서 AI와 조사·탐색 대화를 나눈다 (필요 시 Reference 컨텍스트 주입)
2. 어시스턴트 응답 하단 액션바의 **"Document로 내보내기"** 버튼을 클릭한다
3. 확인 또는 미니 폼 (제목 편집 + Project 할당 선택) 후 생성 확정
4. Document Asset이 생성된다 — 어시스턴트 응답 본문이 초안으로 채워지고, 도메인 모델의 참조 메타데이터가 자동으로 연결된다:
   - `Asset.origin_chat_id` = 현재 Chat의 ID
   - `Message.generated_asset_id` = 새 Document의 ID
   - 대화에서 참조된 Reference들이 Document 메타데이터로 기록 (형식은 Developer D3에서 구체화)
5. 생성 직후 Write 뷰(`/p/[pid]/write/d/[docId]` 또는 `/write/d/[docId]`)로 자동 이동해 편집 대기 상태 (편집 본체는 기능 5)

**이 포워딩 플로우가 "주"인 이유**: 프로젝트 핵심 가치 1번(세 서비스 통합성)을 구체적으로 구현하는 지점이다. "아이디어 서칭(Liner) → 초고 작성(Write)"이 별도 앱 전환 없이 하나의 액션으로 흘러간다.
...
4. **"이 Chat이 만든 Asset도 함께 이동할까요?"** 옵션이 표시된다
...
- **Document Asset CRUD**
  - 생성 경로 **이원화**:
    - (주) **Chat 결과 포워딩**: 어시스턴트 응답 하단 "Document로 내보내기" 액션 → 응답 본문이 초안, `origin_chat_id`·`generated_asset_id` 자동 연결. 시나리오 5-b
    - (보조) **빈 Document 직접 생성**: "새 Document" 버튼 → 제목 입력 + Project 할당 선택. 시나리오 5-a
  - 목록 표시: **사이드바 "최근 기록" 섹션 재사용**(원본 Liner Write 패턴). Write 뷰 라우트(`/p/[projectId]/write` 및 미할당 `/write`) 메인 패널은 빈 상태 CTA + 최근 편집한 Document placeholder 수준으로 경량
  - 삭제: 도메인 모델 삭제 동작 계승
  - **편집은 기능 5 Write 뷰의 몫** — 이 단계에서는 생성(두 경로)·목록·삭제만. `/write/d/[documentId]` 편집 본체는 기능 5에서 채움
...
- **Chat ↔ Project 할당·이동**
  ...
  - "이 Chat이 만든 Asset도 함께 이동할까요?" 옵션 제공 (도메인 모델 행동 규칙)
...
7. 빈 Document Asset을 직접 생성할 수 있다(보조 플로우 / 시나리오 5-a). Project 내 또는 미할당 상태 모두 가능.
8. Chat 어시스턴트 응답 하단 액션바의 "Document로 내보내기" 버튼으로 Document Asset을 **포워딩 생성**할 수 있다(주 플로우 / 시나리오 5-b). 생성된 Document의 `origin_chat_id`·`Message.generated_asset_id`가 자동 연결되고, 생성 직후 Write 뷰로 이동한다.
9. Document Asset 목록이 **사이드바 "최근 기록" 섹션**에 노출된다. Write 뷰 라우트(`/p/[projectId]/write` 및 `/write`) 메인 패널은 빈 상태 CTA 또는 경량 placeholder.
10. Document Asset을 삭제하면 목록에서 사라지고, 도메인 모델 삭제 동작이 올바르게 실행된다.
11. 메시지 전송 전 **입력창 하단 첨부 영역**에서 Reference Asset을 컨텍스트로 선택할 수 있다. 선택된 Reference는 칩(chip) 형태로 표시되고, 내용이 LLM 호출 시 주입된다.
12. Reference를 컨텍스트로 주입한 응답의 `[n]` 출처 배지를 클릭하면, **실제 Reference Asset의 제목·URL·발췌**가 출처 패널에 표시된다. (기능 3 stub 대체)
13. Message의 `referencedAssetIds`에 실제 Reference Asset ID가 저장된다.
14. 미할당 Chat을 Project에 할당할 수 있다. 할당 후 사이드바 Project 트리에 해당 Chat이 나타난다.
15. Chat 이동 시 "이 Chat이 만든 Asset도 함께 이동할까요?" 옵션이 **기본 off** 상태로 표시되고, 사용자 선택에 따라 Asset의 `project_id`가 적절히 업데이트된다.
16. 빈 상태(Asset 0개), 로딩, 삭제 에러 등 모든 상태에 대한 피드백 UI가 존재한다. Asset 빈 상태는 원본의 파일 업로드 박스를 제거하고 **"URL 입력" / "텍스트 입력" 2버튼**만 중앙 배치한다.
17. **T-004 해소**: SSE 에러 후 재시도 시 user 메시지가 DB에 중복 생성되지 않는다. 재시도 흐름은 기존 user 메시지 ID를 재사용하고 assistant 응답만 새로 생성한다.

### 기술·품질 요구

18. Asset 모델이 단일 polymorphic 테이블 + 개별 nullable 컬럼 하이브리드로 Prisma 스키마에 추가되고 마이그레이션이 적용된다 ([ADR-0014](../../architecture/decisions/0014-asset-schema-polymorphic-single-table.md)).
19. 모든 Asset CRUD Server Action이 ADR-0008 패턴(zod + result 객체 + `lib/actions/asset.ts`)을 따른다.
20. 포워딩 생성 Server Action은 트랜잭션으로 Document 생성 + `Message.generated_asset_id` 업데이트를 원자적으로 처리한다.
21. 모든 Server Action과 Route Handler에서 session 기반 소유권 검증이 적용된다. 다른 사용자의 Asset 접근 시 404 반환.
22. 미할당 라우트(`/liner`, `/liner/c/[chatId]`, `/write`, `/write/d/[documentId]`)가 존재하며, 기존 `/p/[projectId]/...` 라우트와 레이아웃 구조가 충돌 없이 공존한다 ([ADR-0015](../../architecture/decisions/0015-unassigned-route-structure.md)).
23. Asset 전용 top-level 라우트(`/assets` 등)가 존재하지 않는다. Asset 접근은 뷰(Liner/Write) 내부 경로로만.
24. 출처 패널이 실 Asset 데이터를 표시하며, stub 하드코딩이 코드베이스에서 제거된다.
25. Reference 컨텍스트 주입 시 시스템 프롬프트에 Reference 내용이 올바르게 포함된다.
26. 타입 체크(`pnpm typecheck`)·린트(`pnpm lint`) 통과.
27. T-001 해소: `color-primary`와 `color-error` 토큰이 `design/design-tokens.md`에 등록된다.
...
| Document 생성 진입점 | **포워딩(주) + 빈 Document 직접 생성(보조)** 이원화. 원본 Liner Write는 Chat-like 진입(빠른 시작 버튼)이지만 통합 앱의 핵심 가치(서비스 간 맥락 연결)를 UI로 실현하려면 Liner 대화 결과를 그대로 Document로 넘기는 포워딩이 주 플로우여야 함. 근거: [`design/references/4-asset/measurements.md`](../../design/references/4-asset/measurements.md) 열린 관찰 (Designer Step B) | **결정됨** (Designer Step B 발견) |
...
| D3 | Document Asset CRUD + 포워딩 | Document 생성(이원화)·목록·삭제 | (i) 빈 Document 생성 Server Action + 미니 모달 UI(제목 + Project 선택). (ii) **Chat 응답 포워딩 Server Action** — 트랜잭션으로 Document 생성 + `Message.generated_asset_id` 업데이트, `origin_chat_id` 자동 설정. Chat 메시지 응답 하단 액션바에 "Document로 내보내기" 버튼 추가. (iii) 사이드바 "최근 기록" 섹션에 Document 목록 노출 (Project 소속 구분). (iv) `/p/[projectId]/write` 메인 패널은 경량 placeholder. 삭제 + 도메인 모델 삭제 동작. `/write/d/[documentId]`는 편집 placeholder(기능 5) | 편집(기능 5), 미할당 라우트 |
...
| D5 | Chat ↔ Project 할당·이동 + Asset 동반 이동 옵션 | 미할당 → Project 이동 동선 | Chat 이동 Server Action (`project_id` 업데이트), "이 Chat이 만든 Asset도 함께 이동?" 옵션 UI(**기본값 off**) + 동반 이동 Server Action, 사이드바 Project 트리 업데이트 | 출처 stub 교체 |
