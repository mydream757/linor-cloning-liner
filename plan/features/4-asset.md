...
| T-001 해소 시점 | 기능 4 Developer 단계 진입 시 원본 라이너 추가 실측 후 확정 | **결정됨** (tech-debt.md T-001 조건, 커밋 `fe1e698`) |
| Write 뷰 Reference 첨부 UI | **기능 4에서 미도입, 기능 5 범위로 이관**. Write 뷰의 AI 호출은 "선택 영역 → AI 수정 제안"(기능 5)이 유일한 진입이고, 그 요청 폼에 Reference 첨부를 1급으로 넣는 것이 자연스럽다. 편집 본체(TipTap) 없이 첨부 UI만 미리 붙이면 동작하지 않는 유령 UI가 된다. `@Reference` 멘션 삽입은 features.md 2차 후보 | **결정됨** (D6 사용자 피드백, 2026-04-22) |
| 사이드바 Reference 진입점 | **D7 범위로 편입**. 현재 `/p/[projectId]/references`는 URL 직접 입력으로만 접근 가능(D2 임시). 사이드바에 "자료" 링크 + 미할당 Reference 페이지(`/references`) 신규 추가. Designer Step C에서 대칭 고려했으나 D2 임시 페이지 처리로 미뤄둠 | **결정됨** (D6 사용자 피드백, 2026-04-22) |
...
| D7 | 종합 검증 + 사이드바 통합 + features.md 갱신 | 기능 4 완료 | **사이드바 Reference 진입점 추가** — 기능 4 D6 완료 후 승격된 결정(§5 참조): (1) `/p/[projectId]/references` Project 스코프 페이지로 이동할 "자료" 링크를 Project 트리 또는 상단 액션에 추가, (2) 미할당 Reference 페이지(`/references`) 신규 생성 — `listUnassignedReferencesByUser` 쿼리는 D2에서 이미 존재, (3) Reference 빈 상태 CTA에서 직접 Reference 추가 진입. **Golden path + 엣지 케이스 수동 검증** — 3 시나리오(5-a/5-b/5-c), Reference 선택 → 프롬프트 주입 → 배지 + 패널, Chat 이동, 미할당 플로우, 재시도 플로우 전부. **타입체크/린트**. **PM 명세 열린 질문 해소 반영** (§7 완결). **T-001(`fe1e698`)·T-004(`eb9b784`) 해소 상태 확인**. **`features.md` 기능 4 상태 "완료"로 갱신** + 기능 5 비고에 "Reference 첨부 UI 편입 예정" 추가 | — |
...
- 0.6 (2026-04-22): D6 완료 시점 결정 2건 + D7 scope 상세화.
  - (1) **Write 뷰 Reference 첨부 UI 기능 5 이관 결정** — Write 뷰의 유일한 AI 호출 시점은 "선택 영역 → AI 수정 제안"(기능 5)이고, 편집 본체(TipTap)가 없는 상태에서 첨부 UI만 있으면 동작하지 않는 유령 UI가 됨. features.md 2차 후보의 `@Reference` 멘션은 기능 5 이후.
  - (2) **사이드바 Reference 진입점 D7 편입** — 현재 `/p/[pid]/references`는 D2 임시로 URL 직접 입력만 가능. "자료" 링크 + 미할당 `/references` 페이지를 D7에서 함께 추가.
  - §5 결정 요약 표에 2건 추가, §8 D7 scope 체크리스트로 상세화 (사이드바 Reference 진입점 작업 3개 세분화 + T-001/T-004 해소 상태 확인 + 수용 기준 검증 범위 명시).
