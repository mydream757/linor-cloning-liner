...
> 참조
> - 기능 명세: [`plan/features/4-asset.md`](../../plan/features/4-asset.md) v0.3
...
### 2-8. Document 생성 — 시나리오 5-a (빈 Document 미니 모달)

보조 플로우. 사이드바 "새 Document" 버튼 또는 Write 뷰 빈 상태 CTA에서 진입.

```
┌────────────────────────────────────────────────────────────────────────┐
│                          화면 전체 backdrop                               │
│  ┌───────────────────────────────────────────┐                         │
│  │  새 Document                  [×]          │  ← 400px (measurements)  │
│  │                                           │                         │
│  │  제목 *                                    │                         │
│  │  ┌────────────────────────────────────┐   │                         │
...
│  │  Project (선택)                            │                         │
│  │  ┌────────────────────────────────────┐   │                         │
│  │  │  Project 선택 (미할당 유지 가능)      ▼  │  ← h 40px, radius-md    │
│  │  └────────────────────────────────────┘   │                         │
│  │                                           │                         │
│  │                          [취소] [만들기]  │                         │
│  └───────────────────────────────────────────┘                         │
...
| 요소 | 토큰 | 치수 | 비고 |
|---|---|---|---|
| 모달 컨테이너 | `color-bg-primary` + `shadow-dialog` | w 400px, `radius-dialog` (measurements) | — |
| 모달 내부 padding | — | 24px (measurements) | — |
| 제목 입력 필드 | `color-text-primary` + `color-border-normal` | h 52px, `radius-md` | 필수. 비어있으면 "만들기" 비활성 |
| Project 선택 드롭다운 | `color-text-primary` + `color-border-normal` | h 40px, `radius-md` (measurements) | "미할당" 옵션 포함. 선택 없이도 만들기 가능 |
| 취소 버튼 | `color-text-primary` + `color-border-normal` | 80×40px, `radius-md` | — |
| "만들기" 버튼 (활성) | `color-primary` bg | 80×40px, `radius-md` | 제목 입력 시 활성 |
| "만들기" 버튼 (비활성) | `color-surface-overlay` bg | 80×40px, `radius-md` | — |

**상태**: Reference 추가 모달(§2-3)과 동일한 상태 패턴.

**전역 토큰 참조**: `color-bg-primary`, `color-text-primary`, `color-border-normal`, `color-primary`, `color-surface-overlay`, `radius-md`, `radius-dialog`, `shadow-dialog`

---

### 2-9. Document 생성 — 시나리오 5-b (Chat 응답 "Document로 내보내기" 액션)

주 플로우. 어시스턴트 응답 하단 액션바 "Document로 내보내기" 버튼에서 진입.

```
어시스턴트 응답 하단 액션바:
┌──────────────────────────────────────────────────────────────────────────┐
│  📋  2개의 출처  [Document로 내보내기]  👍 👎 ⋯                           │
└──────────────────────────────────────────────────────────────────────────┘

"Document로 내보내기" 클릭 → 응답 아래 inline 미니 확인 폼 등장:
┌──────────────────────────────────────────────────────────────────────────┐
│  어시스턴트 응답 텍스트 (위에 보임)                                              │
├──────────────────────────────────────────────────────────────────────────┤
│  Document로 내보내기                                                        │
│  ┌────────────────────────────────────────────────────────────────────┐  │
│  │  AI 응답 첫 줄에서 자동 생성된 제목                     [수정 가능]    │  │
│  └────────────────────────────────────────────────────────────────────┘  │
│  Project (선택)  ▼                                                        │
│                                          [취소] [Document 생성]          │
└──────────────────────────────────────────────────────────────────────────┘
...
| 요소 | 토큰 | 치수 | 비고 |
|---|---|---|---|
| 인라인 폼 컨테이너 | `color-bg-secondary` + `color-border-subtle` border-top | — | 응답 바로 하단 inline. 별도 모달 없음. 어시스턴트 응답 맥락을 유지 |
| 폼 내부 padding | — | 16px 24px (measurements 준용) | — |
| 제목 입력 필드 | `color-text-primary` + `color-border-normal` | h 40px, `radius-md` | 초기값: 어시스턴트 응답 첫 문장 앞 40자 자동 입력. 사용자가 수정 가능 |
| Project 선택 드롭다운 | `color-text-primary` + `color-border-normal` | h 36px, `radius-md` | 미할당 기본값. 현재 뷰의 Project 사전 선택 |
| 취소 버튼 | `color-text-secondary` | 텍스트 버튼 | 폼 닫힘. 응답에 변화 없음 |
| "Document 생성" 버튼 | `color-primary` bg | 높이 36px, `radius-md` | 클릭 시 트랜잭션: Document 생성 + `origin_chat_id` 설정 + `generated_asset_id` 업데이트 → Write 뷰 자동 이동 |
| 생성 중 상태 | "Document 생성" 버튼 비활성 + 스피너 | — | — |

**포워딩 자동 연결 정보 (도메인 모델 참조)**:

- `Asset.origin_chat_id` = 현재 Chat ID (자동)
- `Message.generated_asset_id` = 생성된 Document ID (트랜잭션 처리)
- 선택된 Reference들의 ID가 Document 메타데이터에 기록

**상태**:

| 상태 | 변화 |
|---|---|
| 폼 기본 | 응답 아래 inline 등장, 제목 자동 입력됨 |
| 생성 중 | "Document 생성" 버튼 비활성 + 스피너 |
| 생성 완료 | Write 뷰 자동 이동 (`/write/d/[docId]` 또는 `/p/[pid]/write/d/[docId]`) |
| 생성 실패 | 폼 내 인라인 에러 + 재시도 가능 |

**전역 토큰 참조**: `color-bg-secondary`, `color-border-subtle`, `color-text-primary`, `color-text-secondary`, `color-border-normal`, `color-primary`, `radius-md`
...
### (2) Document "포워딩" 주 플로우 UI — Chat 응답 액션바 통합

Document 생성 진입점이 이원화(주: 포워딩, 보조: 빈 Document)되는데, 포워딩 UI를 별도 모달 대신 **응답 하단 인라인 폼**으로 구현한다.

| 대안 | 장점 | 단점 | 선택 여부 |
|---|---|---|---|
| **인라인 폼 (채택)** | 포워딩 대상 응답이 눈에 보이는 상태에서 제목/Project를 결정 — 맥락 분리 없음 | 폼이 응답 아래 공간을 차지해 메시지 영역이 길어질 수 있음 | **채택** |
| 별도 모달 | 레이아웃 변화 없음 | 포워딩 대상 응답이 모달 뒤로 가려짐. 맥락 전환 비용 | 기각 |
| 바로 Write 뷰 이동 후 편집 | 클릭 즉시 이동 — 빠른 피드백 | 제목·Project를 못 정하고 이동. Write 뷰에서 다시 설정해야 함 | 기각 |

**근거**: 이 프로젝트의 핵심 가치 1번(통합성)이 "아이디어 서칭(Liner) → 초고 작성(Write)"이 하나의 맥락에서 흘러야 함을 의미한다. 포워딩 시 어시스턴트 응답 내용을 보면서 제목을 확인·수정하는 것이 자연스럽고, 인라인 폼은 이 맥락을 유지하는 최소 단위의 진입점이다.
